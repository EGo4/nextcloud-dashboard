...
session_write_close(); 
if (!defined('NC_USER')) {
    echo "<div class='p-4 text-red-600 text-sm'>Bitte lade die Seite neu, deine Sitzung ist abgelaufen.</div>";
    exit;
}
...
<div id="nc-comments-box" hx-disinherit="*" class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 h-full flex flex-col">
   ...
   <form hx-post="modules/comments.php?folder=<?= urlencode($currentFolder ?? '') ?>&image=<?= urlencode($currentImage ?? '') ?>" 
      hx-target="#nc-comments-box" 
...
<script>
    (function() {
        // Sicherheits-Check: Ist Tribute geladen?
        if (typeof Tribute === 'undefined') return;

        var nextcloudUsers = <?= $ncUsersJson ?? '[]' ?>;
        // Wir suchen explizit im neu geladenen Container nach dem Feld
        var commentsBox = document.getElementById('nc-comments-box');
        var inputField = commentsBox ? commentsBox.querySelector('#commentInput') : null;

        if (inputField && nextcloudUsers.length > 0) {
            var tribute = new Tribute({
                values: nextcloudUsers,
                selectTemplate: function (item) { return '@' + item.original.value; },
                menuItemTemplate: function (item) { return '<i class="fas fa-user-circle text-gray-400 mr-2"></i>' + item.string; },
                noMatchTemplate: function () { return '<span style="visibility: hidden;"></span>'; }
            });
